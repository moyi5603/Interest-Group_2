...
> **加入与报名解耦**：员工可直接报名活动，**不必先加入小组**（见 §7.10）。
...
> **说明**：MVP 的「AI 推荐」指**可解释的规则排序 + 理由文案**，不使用向量检索或大模型做排序。对话助手里的「推荐跑步小组」也调用同一套 `recommendGroups` 结果，仅展示形式不同。

### 5.1 小组推荐

#### 5.1.1 推荐什么

向当前员工推荐其**尚未加入、且非本人创建**的**活跃小组**，每条结果包含：

- 小组卡片（名称、封面、标签、成员数等）
- **1 条主推荐理由**（卡片上展示 `reasons[0]`，其余理由可后续扩展）
- 列表区摘要文案（如「根据你的兴趣，为你挑选了 3 个还没加入的小组」）

**出现位置**：兴趣小组首页「AI 推荐」、发现页默认 Tab、对话意图 `recommend_group`、首页「换一批」。
...
| 员工已是成员 | 不推荐 |
| 员工是该小组创建人（owner） | 不推荐 |
| 可见性不满足（如 `dept_only` 且非本部门） | 不进候选池 |

可见性规则见 §6.3 / `canViewGroup`：公开组全员可见；部门组仅对应部门；邀请制仅成员可见（故通常不会出现在推荐池）。
...
打分阶段写入的**主理由**（按命中情况叠加，展示时取第一条或合并策略以 UI 为准）：
...
首页等对同一排序列表使用 **offset 滑动窗口**：在已排序的完整列表上按 `offset` 截取下一屏，避免每次都返回完全相同的前 N 个（列表长度不足一页时循环范围相应缩小）。

#### 5.1.6 与对话助手的关系

用户说「推荐跑步小组」等时，对话层做**关键词意图识别**（mock），命中 `recommend_group` 后调用**同一套** `recommendGroups(viewerId, limit)`，以小组卡片列表回复；排序与理由与首页一致，不是另一套黑盒推荐。
...
| 非小组成员 | 在可见范围内可浏览活动、**直接报名**（§7.10）；无「发布活动」 |
...
### 7.10 活动报名与小组 membership 解耦（2026-05-27）

**决策**：**未加入小组的员工也可以报名该小组下的活动**；加入小组与报名活动为两条独立路径。

| 维度 | 规则 |
|------|------|
| 报名前提 | 满足活动可见性（小组 `visibility` + 活动 `published`）、名额与截止时间等业务规则即可 |
| 不要求 | 报名前**不必**「加入小组」、不必存在 `GroupMembership` |
| 加入小组 | 仍为可选：便于持续关注小组动态、满足 `invite_only` 小组的发现规则等 |
| 退出小组 | 退出时取消该小组内活动报名（`leaveGroup`）；与「未加入仅报名」不冲突 |
| 发布活动 | 仍仅组长（§7.5），与报名权限无关 |

**可见性（与报名配合）**：

- `public` / `dept_only`：非成员可浏览活动详情并报名（在部门范围内）。
- `invite_only`：非成员通常无法进入小组详情；若通过活动深链进入详情，MVP 以能打开的活动页为准是否可报。

**原型实现**：`enrollActivity` / `enrollOccurrences` **不校验** `isMember`；`ActivityDetail` 报名按钮对非成员同样展示（仅受终止、满员、截止等约束）。
...
| ActivityDetail | 时间、地点、报名（**无需先加入小组**，§7.10）、场次列表（系列/周期） |
...
| 2026-05-27 | §7.10：未加入小组也可报名活动；加入与报名解耦；原型 `enrollActivity` 不校验 membership |
