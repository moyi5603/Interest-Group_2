...
1. **报名前置加入**：必须先加入活动所属小组才能报名。自由加入小组按钮「报名并加入小组」一键完成；审核加入小组为硬门槛——先「申请加入」进入待审核（`group.pending`），与 PC 待审核队列联动，审核通过后方可报名。退出小组连带取消该组报名。详见 `docs/superpowers/specs/2026-06-04-join-required-to-signup-design.md`。
